...
class MultiAttack(Attack):
    ...
    def __init__(self, attacks, verbose=False, attack_right=False,attack_protected=False,):
        super().__init__("MultiAttack", attacks[0].model, attack_right,attack_protected)
    ...
    def forward(self, images, labels,mask_protected=None):
        ...
        for _, attack in enumerate(self.attacks):
            if mask_protected is None:
                adv_images = attack(images[fails], labels[fails],None)
            else:
                adv_images = attack(images[fails], labels[fails],mask_protected[fails])
