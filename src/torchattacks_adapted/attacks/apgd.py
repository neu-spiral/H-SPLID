...
class APGD(Attack):
    ...
    def __init__(
        ...
        super().__init__("APGD", model, attack_right,attack_protected)
        self.eps = eps
        self.steps = steps
        self.norm = norm
    ...
    def forward(self, images, labels, mask_protected=None):
        ...
        _, adv_images = self.perturb(images, labels,mask1,mask2, cheap=True)
    ...
    def attack_single_run(self, x_in, y_in, mask1=None, mask2=None):
        ...
        if self.norm == "Linf":
            ...
            x_adv = x.detach() + mask1*self.eps * torch.ones([x.shape[0], 1, 1, 1]).to(
                self.device
            ).detach() * t / (
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                masked=x_adv* mask1 + x* mask2
                logits = self.get_logits(masked)
    ...
    def perturb(self, x_in, y_in,mask1=None,mask2=None, best_loss=False, cheap=True):
        assert self.norm in ["Linf", "L2"]
        x = x_in.clone() if len(x_in.shape) == 4 else x_in.clone().unsqueeze(0)
        y = y_in.clone() if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)

        adv = x.clone()
        acc = self.get_logits(x)
        if type(acc) == tuple:
                acc = acc[0]
        acc=acc.max(1)[1] == y
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        (
                            ...
                            x_to_fool, y_to_fool,mask1_to_fool, mask2_to_fool,
                        )  # nopep8
