...
class Square(Attack):
    ...
    def __init__(
        ...
        super().__init__("Square", model,attack_right,attack_protected)
        self.norm = norm
    ...
    def attack_single_run(self, x, y, mask1=None, mask2=None):
        with torch.no_grad():
            adv = x.clone()
            c, h, w = x.shape[1:]
            n_features = c * h * w
            ...
            if self.norm == "Linf":
                #TT
                x_best = torch.clamp(
                    x + mask1*self.eps * self.random_choice([x.shape[0], c, 1, w]), 0.0, 1.0
                )
                ...
                for i_iter in range(self.n_queries):
                    ...
                    m1_new=self.check_shape(mask1[idx_to_fool])
                    m2_new=self.check_shape(mask2[idx_to_fool])
    ...
    def perturb(self, x, y=None,mask1=None,mask2=None,):
        """
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                _, adv_curr = self.attack_single_run(x_to_fool, y_to_fool,mask1_to_fool, mask2_to_fool,)
