...
class FAB(Attack):
    ...
    def __init__(
        ...
        super().__init__("FAB", model,attack_right,attack_protected)
        self.norm = norm
    ...
    def forward(self, images, labels,mask_protected=None):
        ...
        adv_images = self.perturb(images, labels,mask1,mask2)
    ...
    #TT
    def get_diff_logits_grads_batch(self, imgs, la,mask1=None,mask2=None,im0=None):
        im = imgs.clone().requires_grad_()
        with torch.enable_grad():
            masked=im* mask1 + im0* mask2
        with torch.enable_grad():
            y = self.get_logits(masked)
        ...
        print(df.shape)
        for i in range(10):
            print(torch.count_nonzero(dg[:,i,:,:,:]*mask2))
        sys.exit()
        '''
        ...
        return df, dg
    #TT
    def get_diff_logits_grads_batch_targeted(self, imgs, la, la_target,mask1=None,mask2=None,im0=None):
    ...
    def attack_single_run(self, x, y=None, mask1=None, mask2=None, use_rand_start=False):
        """
        ...
        #TT
        m1_new=mask1[pred].detach().clone()
        m2_new=mask2[pred].detach().clone()
        ...
        if self.attack_right:
            mask1=torch.ones_like(im2)
            mask1[:,:,:,:32] = 0
            mask2=torch.ones_like(im2)
            mask2[:,:,:,32:] = 0
        else:
            mask1=torch.ones_like(im2)
            mask2=torch.zeros_like(im2)
        '''  
        ...
        while counter_restarts < 1:
            ...
            while counter_iter < self.steps:
                with torch.no_grad():
                    df, dg = self.get_diff_logits_grads_batch(x1, la2,m1_new,m2_new,im2)
                    if self.norm == "Linf":
                        dist1 = df.abs() / (
                            1e-12
                    ...
                    #TT
                    x1 = (
                        (x1 + m1_new*self.eta * d1) * (1 - alpha)
                        + (im2 + m1_new*d2 * self.eta) * alpha
                    ).clamp(0.0, 1.0)
    ...
    def attack_single_run_targeted(self, x, y=None, mask1=None, mask2=None, use_rand_start=False):
        """
        :param x:    clean images
        :param y:    clean labels, if None we use the predicted labels
        """
            ...
            while counter_iter < self.steps:
                with torch.no_grad():
                    df, dg = self.get_diff_logits_grads_batch_targeted(
                        x1, la2, la_target2,m1_new,m2_new,im2
                    )
                    if self.norm == "Linf":
                        dist1 = df.abs() / (
                            1e-12
    ...
    def perturb(self, x, y, mask1,mask2):
        adv = x.clone()
        with torch.no_grad():
            ...
            def inner_perturb(targeted):
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        #TT
                        mask1_to_fool, mask2_to_fool = (
                            mask1[ind_to_fool].clone(),
                            mask2[ind_to_fool].clone(),
                        )
                        if targeted:
                            adv_curr = self.attack_single_run_targeted(
                                x_to_fool, y_to_fool,mask1_to_fool, mask2_to_fool, use_rand_start=(counter > 0)
                            )
                        else:
